...
def SheetGet():
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name("CavTest.json", scope)
...
InitialTicketCells = ["AO","CLERK", "TURN TIME", "", "TOTAL TICKETS", '=COUNTA(A:A)-1', "", "CLERK","TOTAL TICKETS","AVG TURN TIME", "", "OVERALL AVG TURN TIME"]

cell_listInitial = Sheet.range("A1:L1")
...
cell_listTicketTrackerFormuler = Sheet.range("E2:F{}".format(len(CurrentAOs)))
...
for AO in range(len(CurrentAOs)): #Loop for each AO with formula to count how many tickets are part of it
    ...
    FormulaList.append("=COUNTIF(A:A,E{})".format(AO+2))
...
cell_listClerkTrackerFormuler = Sheet.range("H2:J{}".format(len(Clerks)))
...
for Clerk in range(len(Clerks)):
    ...
    ClerkList.append('=COUNTIF(B:B,H{})'.format(Clerk+2))
    ClerkList.append('=AVERAGEIF(B:B,H{},C:C)'.format(Clerk+2))

f.write("\n,,,,TOTAL AVG TURN TIME\n") #Overall turn time
f.write(',,,,"=AVERAGE(C:C)"\n')

Sheet.update_cell(2,12,"=AVERAGE(C:C)")
...
fmtOK = cellFormat(backgroundColor = color(1,1,1))

formatlisting = [('A1:M1', fmtheader)]
...
for cellA in cell_listA:
    ...
    if Check >= 24:
        #format_cell_range(Sheet, 'C{}'.format(x), fmtovertime)
        formatlisting.append(('C{}'.format(x+2),fmtovertime)) 
        Check = 0
    else:
...
format_cell_range(Sheet,'A2:J{}'.format(len(Result)), center)
